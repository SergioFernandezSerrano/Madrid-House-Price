...
Convertimos en NA la casa de 41 habitaciones (probablemente fueran 4 y está mal imputado), las que tienen menos de 3 m2 (probablemente mal imputados al usar el punto para separar los miles) y la casa con un precio de 725 (claramente equivocado).
...
```{r}
boxplot(mhp_train$price, main = 'Precio', ylab = "Valores", col = "#0BB363", las = 2)
...
ggplot(mhp_train, aes(rooms)) +
  ...
  geom_density(alpha=.5, fill = "red") +
  labs(x = "Habitaciones", y = "Nº de viviendas", title = "Viviendas por número de habitaciones") +
  theme(plot.title = element_text(hjust = 0.5)) +
scale_x_continuous(breaks = seq(0, max(mhp_train$rooms), by = 1))
```
...
```{r}
with(mhp_train, CrossTable(exterior, elevator, format = 'SPSS'))
```

```{r}
with(mhp_train, ftable(exterior, elevator, garage))
```
...
mhp_train %>%
  ...
  ggtitle("Precio medio por distrito y tipo de casa") + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) + labs(x = "distrito", y = "precio medio")
```
...
mhp_train %>%
  ...
  geom_bar(stat = "identity", position = "dodge") + 
  ggtitle("Precio medio por distrito y ascensor") + theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) + labs(x = "distrito", y = "precio medio")
```
...
mhp_train %>%
  ...
  geom_bar(stat = "identity", position = "dodge") + 
  ggtitle("Precio medio por distrito y garaje") + theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 90, hjust = 1)) + labs(x = "distrito", y = "precio medio")
```
