...
walmart_stats_temperature <- walmart %>%
  group_by(Store) %>%
  summarise(min = formatCurrency(min(Temperature)),
            max = formatCurrency(max(Temperature)),
            median = formatCurrency(median(Temperature)),
            mean = formatCurrency(mean(Temperature)),
            sd = formatCurrency(sd(Temperature)))

# Calcular estadísticas de Walmart para la variable Fuel_Price
walmart_stats_fuel <- walmart %>%
  group_by(Store) %>%
  summarise(min = formatCurrency(min(Fuel_Price)),
            max = formatCurrency(max(Fuel_Price)),
            median = formatCurrency(median(Fuel_Price)),
            mean = formatCurrency(mean(Fuel_Price)),
            sd = formatCurrency(sd(Fuel_Price)))

# Calcular estadísticas de Walmart para la variable CPI
walmart_stats_cpi <- walmart %>%
  group_by(Store) %>%
  summarise(min = formatCurrency(min(CPI)),
            max = formatCurrency(max(CPI)),
            median = formatCurrency(median(CPI)),
            mean = formatCurrency(mean(CPI)),
            sd = formatCurrency(sd(CPI)))

# Calcular estadísticas de Walmart para la variable Unemployment
walmart_stats_unemployment <- walmart %>%
  group_by(Store) %>%
  summarise(min = formatCurrency(min(Unemployment)),
            max = formatCurrency(max(Unemployment)),
            median = formatCurrency(median(Unemployment)),
            mean = formatCurrency(mean(Unemployment)),
            sd = formatCurrency(sd(Unemployment)))
...
walmart_stats <- walmart %>%
  group_by(Store) %>%
  summarise(min = formatCurrency(min(Weekly_Sales)),
            max = formatCurrency(max(Weekly_Sales)),
            median = formatCurrency(median(Weekly_Sales)),
            mean = formatCurrency(mean(Weekly_Sales)),
            sd = formatCurrency(sd(Weekly_Sales)),
            sum = formatCurrency(sum(Weekly_Sales)))
...
Antes de graficar las series temporales de nuestro dataset vamos a sacar por tienda las 3 mayores ventas y el restos de las variables asociadas a las mismas, principalmente para identificar si hay algún patrón como que todas se producen en las mismas fechas, que todas tienen unos índeces CPI concretos o que están sujetas a unas tasas de desempleo específicas...
...
Se observa claramente como 6 de esos 10 días con más ventas son las semanas de Navidad, seguido por Acción de Gracias en noviembre.
...
Dado que tenemos datos semanales y parece existir a simple vista un claro patrón estacional anual durante las semanas finales de Noviembre y Diciembre donde hay un considerable aumento de las ventas; **NO** reservamos ninguna parte de dataset para test pues este finaliza el 26/10/2012, justo antes de los periodos con mayor consumo en los establecimientos, precisamente los que queremos predecir.
