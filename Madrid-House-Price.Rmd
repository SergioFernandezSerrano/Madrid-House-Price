...
La presencia de ceros en la matriz de rotación en las últimas columnas PC6,PC7 y PC8 indican que estas variables originales no contribuyen significativamente a la variación en los datos, por lo que se podrían eliminar del análisis.
...
summary(pca_result)
```
Observamos que con PC5 ya llegamos al 100% de la proporción de varianza explicada per vemos algunos saltos interesantes. Para ver cuantas componentes explican el modelo hacemos la regla de codo.
...
```
Al observar el gráfico vemos que la forma del codo sería en la 5, puesto que de la 4 a la 5 todavía hay un salto grande.
...
summary(glm_mhp)
```
El modelo de regresión logística ajustado incluye las variables 'exterior', 'garage', 'elevator' y 'rooms' como predictores de la variable de respuesta binaria 'cara'. Todas las variables predictoras son estadísticamente significativas en la predicción de 'cara', lo que indica que tienen un efecto relevante en la probabilidad de que un dato sea clasificado como 'cara'.

El modelo completo explica una parte significativa de la variabilidad en los datos, como lo demuestra la reducción en la devianza residual en comparación con la devianza nula. Además, el AIC sugiere que el modelo tiene un buen equilibrio entre complejidad y ajuste a los datos.
...
matriz_confusion_glm <- table(glm_mhp$model$cara, predicciones_glm,
...
```{r}
train_glm$pred_prob <- predict(glm_mhp, type = "response")
train_glm_long <- train_glm %>%
  gather(key = "variable", value = "value", exterior, garage, elevator, rooms) %>%
  mutate(variable = as.factor(variable))
ggplot(train_glm_long, aes(x = value, y = pred_prob)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, color = "blue") +
  facet_wrap(~variable, scales = "free_x") +
  ylim(0, 1) +
  labs(x = "Valor de la variable independiente", y = "Probabilidad predicha", title = "Gráficos de regresión logística")
```
