...
Comenzamos con un modelo muy simple basado en la idea del **Perceptrón**; una de las redes neuronales más simples donde sola capa con tantas neuronas como salidas sean precisas y cada neurona tiene tanas entradas como dimensión del patrón de entrada y la función de activación es la función umbral.

```{r}
# Crear un dataframe con los datos de entrenamiento (mhp_train)
train_data <- mhp_train[, c("price", "rooms", "m2", "elevator", "garage")]
train_data$garage <- as.numeric(train_data$garage)
train_data$elevator <- as.numeric(train_data$elevator)

# Normalizar los datos de entrenamiento
train_data_norm <- scale(train_data)

# Crear una fórmula para el modelo de red neuronal
formula <- as.formula("price ~ rooms + m2 + elevator + garage")

# Entrenar el modelo de perceptrón simple
perceptron_model <- neuralnet(formula, data = train_data_norm, hidden = c(1))

# Imprimir el modelo
print(perceptron_model)
```

Los sedultados obtenidos nos revelan lo siguiente:

-   **Error**: El error obtenido por el modelo es de 1028.128. Este valor indica la diferencia promedio entre los valores reales y los valores predichos por el modelo. Un valor bajo de error indica un buen ajuste del modelo.

-   **Reached Threshold**: El valor obtenido es 0.009869146. Indica la fracción de veces que se alcanzó el umbral durante el proceso de entrenamiento. Un valor cercano a cero indica que el modelo alcanzó el umbral deseado con poca frecuencia.

-   **Steps**: El número de pasos requeridos para entrenar el modelo es de 5573. Este valor representa la cantidad de iteraciones realizadas durante el proceso de entrenamiento.

-   **Intercept.to.1layhid1**: El valor obtenido es 1.294984. Indica el peso o coeficiente asociado entre el intercepto (término constante) y la conexión con el primer nodo de la capa oculta. Este valor representa la importancia relativa de la conexión en el modelo.

-   **rooms.to.1layhid1**: El valor obtenido es 0.1210526. Indica el peso o coeficiente asociado entre la variable "rooms" y la conexión con el primer nodo de la capa oculta. Este valor representa la influencia de la variable "rooms" en la salida del primer nodo de la capa oculta.

-   **m2.to.1layhid1**: El valor obtenido es -0.9165479. Indica el peso o coeficiente asociado entre la variable "m2" y la conexión con el primer nodo de la capa oculta. Este valor representa la influencia de la variable "m2" en la salida del primer nodo de la capa oculta.

-   **elevator.to.1layhid1**: El valor obtenido es 0.1926677. Indica el peso o coeficiente asociado entre la variable "elevator" y la conexión con el primer nodo de la capa oculta. Este valor representa la influencia de la variable "elevator" en la salida del primer nodo de la capa oculta.

-   **garage.to.1layhid1**: El valor obtenido es -0.08063252. Indica el peso o coeficiente asociado entre la variable "garage" y la conexión con el primer nodo de la capa oculta. Este valor representa la influencia de la variable "garage" en la salida del primer nodo de la capa oculta.

-   **Intercept.to.price**: El valor obtenido es 4.284198. Indica el peso o coeficiente asociado entre el intercepto (término constante) y la variable de salida "price". Este valor representa la importancia relativa del intercepto en la predicción del precio.

-   **1layhid1.to.price**: El valor obtenido es -5.580131. Indica el peso o coeficiente asociado entre la salida del primer nodo de la capa oculta y la variable de salida "price". Este valor representa la influencia de la salida del primer nodo de la capa oculta en la predicción del precio.

\

Basandonos en otros modelos de redes neuronales utilizamos primero un modelo lineal para determinar las variables más relevantes a  utilizar en el modelo.

```{r}
mhp$elevator <- as.numeric(mhp$elevator)
mhp$garage <- as.numeric(mhp$garage)
mhp$elevator <- as.numeric(mhp$elevator)
lm_mhp <- lm(price ~ exterior + rooms + m2 + elevator + garage, data=mhp)
summary(lm_mhp)
```

Como el R2 es 71%, aparentemente el modelo lineal explica bastante de la variable respuesta. La variables exterior no parece tener gran relación lineal con la variable respuesta. 
...
El comando  plot(lm_mhp) nos muestra secuencialmente todos los gráficos de diagnosis del modelo. Podemos presentarlos de uno en uno así:

```{r}
res_plot1 <- plot(lm_mhp,1)
res_plot2 <- plot(lm_mhp,2)

grid.arrange(res_plot1, res_plot2, ncol=2)
```

Se evidencia cierta heterocedasticidad, lo cual invalida de entrada el modelo de regresión para este problema; la relación entre el precio de las casas y las variables explicativas es no lineal.

```{r}
mhp.nnet <- nnet(price ~ rooms + m2 + elevator + garage, data = mhp, rang=0.1,size=8,linout=T,maxit=1000)
```

```{r}
red.mhp = neuralnet(price ~ rooms + m2 + elevator + garage, data = mhp,hidden=2,rep=1)
#plot(red.mhp)
```
...
Continuamos seleccionando as variables "rooms", "m2", "elevator" y "garage" como características de entrada (variables independientes) y "price" como la variable objetivo (variable dependiente) que queremos predecir. 

Hemos normalizado los datos de entrenamiento utilizando la función scale() para asegurarnos de que todas las variables tengan la misma escala.

El argumento hidden en la función neuralnet() especifica la estructura de capas ocultas de la red neuronal. En este caso, hemos utilizado dos capas ocultas con 5 y 3 nodos respectivamente.
...
neuralnet_model <- neuralnet(formula, data = train_data_norm, hidden = c(5, 3))
...
El modelo SARIMA (Seasonal Autoregressive Integrated Moving Average) es una extensión del modelo ARIMA que tiene en cuenta tanto la tendencia y la estacionalidad de la serie, así como la autocorrelación y la variabilidad aleatoria.

El término SARIMA se desglosa de la siguiente manera:

-   **S**: Representa la componente estacional de la serie. Indica la periodicidad de la estacionalidad en la serie, es decir, la frecuencia con la que se repite el patrón estacional.
-   **AR**: Significa Autoregressive, se refiere a la componente autorregresiva del modelo. Representa la relación lineal entre los valores pasados y presentes de la serie.
-   **I**: Significa Integrated, se refiere a la componente integrada del modelo. Indica la cantidad de veces que es necesario diferenciar la serie para obtener una serie estacionaria en media.
-   **MA**: Significa Moving Average, se refiere a la componente de media móvil del modelo. Representa la relación lineal entre los errores pasados y presentes del modelo.

El modelo SARIMA se especifica mediante tres conjuntos de parámetros: p, d, q, P, D, Q, y s.

-   **p**: Es el orden del componente autorregresivo (AR), es decir, el número de rezagos pasados que se tienen en cuenta para predecir el valor actual.
-   **d**: Es el orden de diferenciación (Integrated), es decir, el número de veces que se diferencia la serie para obtener una serie estacionaria en media.
-   **q**: Es el orden del componente de media móvil (MA), es decir, el número de errores pasados que se tienen en cuenta para predecir el valor actual.
-   **P**: Es el orden del componente autorregresivo estacional (SAR), es similar al parámetro p pero aplicado a los rezagos estacionales.
-   **D**: Es el orden de diferenciación estacional (Seasonal Integrated), similar al parámetro d pero aplicado a las diferencias estacionales.
-   **Q**: Es el orden del componente de media móvil estacional (SMA), similar al parámetro q pero aplicado a los errores estacionales.
-   **s**: Es la periodicidad estacional de la serie, indica el número de períodos en una temporada completa.
...
<u>**Test de Autocorrelaciones**</u>

```{r}

```

<u>**Test de Homocedasticidad**</u>

```{r}

```

<u>**Test de Normalidad**</u>
