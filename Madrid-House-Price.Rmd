...
Estos resultados muestran el rendimiento del modelo de Random Forest para diferentes valores del hiperparámetro **mtry** que es el número de predictores que se utilizan en cada árbol de decisión. Se han probado 3 valores diferentes: 2, 4 y 7. y para cada valor de mtry se muestran precisiones cercanas al 90% como en el modelo inicial. K-fold validation nos indica que la mayor preción accuracy se obtiene con tan solo dos predictores, no hay necesidad de más.
...
```{r}
# fórmula
formula_svm <- precio_bin ~ rooms_estandarizado + m2_estandarizado + garage + elevator + tipo_casa + precio_distrito
...
Le realizó la validación cruzada con 6 predictores y 10 pliegues alcanzándose una precisión del 90.10% y un Kappa del 80.80% mostrando una alta concordancia entre las predicciones y los valores reales ajustados por el azar.

### N-fold Cross Validation

```{r}
# Cambia los niveles de la variable de clase 'precio_bin' a nombres válidos
mhp_train$precio_bin <- factor(mhp_train$precio_bin, labels = c("Level0", "Level1"))
...
N_F_modelo_svm_cv <- train(precio_bin ~ rooms_estandarizado + m2_estandarizado + garage + elevator + tipo_casa + precio_distrito,
                       data = mhp_train,
                       method = "svmRadial",
                       trControl = control,
                       preProcess = c("center", "scale"),
...
```
Le realizó la validación cruzada con 6 predictores y 10 pliegues alcanzándose una precisión del 90.20% y un Kappa del 80.47%. 

# Elección punto de corte

Nos disponemos a calculas las curvas ROC para cada uno de los modelo y los visualizamos en un mismo plot para mejor comparabilidad.
...
# KNN
#probs_knn <- predict(prediccion_knn5_train, mhp_test, type = "prob")[, 2]
...
# SVM
probs_svm <- predict(modelo_svm, mhp_test[, c("rooms_estandarizado", "m2_estandarizado", "garage", "elevator", "tipo_casa", "precio_distrito")], probability = TRUE)
