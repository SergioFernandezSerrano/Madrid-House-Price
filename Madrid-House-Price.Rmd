...
A continuación, la descripción de cada una de las variables:
  ...
  - **Date**: La semana de las Ventas. Tiene el formato dd-mm-yyyy. Comienza el 05-02-2010
...
Comenzamos con unos gráficos simples por variable para ver como se comportan los datos.

\

```{r}
plot0 <- ggplot(walmart, aes(x = as.factor(Store), y = Weekly_Sales, fill = as.factor(Store))) +
...
Parece que dependiendo de la región donde se encuentre la tienda hay una gran dispersión de gasto en una cesta de la compra promedio y la tasa de desempleo también copntibuye a la misma. El único inconveniente es que a través de este dataset desconocemos las regiones de EEUU donde se sitúan las tiendas Walmart.

```{r}
variables <- c("Weekly_Sales", "Temperature", "Fuel_Price", "CPI", "Unemployment", "Holiday_Flag")

subset_walmart <- walmart[, variables]

ggpairs(subset_walmart)
```

```{r}
ggplot(walmart, aes(x = Date, y = Weekly_Sales, color = as.factor(Store))) +
  geom_line() +
  labs(x = "Date", y = "Weekly Sales", color = "Store") +
  theme_minimal()
```
