...
mhp_train$precio_bin <- as.factor(mhp_train$precio_bin)
```

Aplicamos los cambios también a test.

```{r}
# Caategorizamos house_type
mhp_test <- mhp_test %>%
  mutate(tipo_casa = case_when(
    grepl("casa|chalet", house_type, ignore.case = TRUE) ~ "casa",
    grepl("\\bplanta\\s*-?1\\b|semi-?sotano|bajo|sotano", house_type, ignore.case = TRUE) ~ "sotano",
...
mhp_test$precio_bin <- as.factor(mhp_test$precio_bin)
```
...
```{r}
mhp_train$cara <- ifelse(mhp_train$cara == "precio_bin", 1, 0)
```
