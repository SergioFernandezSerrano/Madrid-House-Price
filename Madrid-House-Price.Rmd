...
\#.
Para el posterior análisis agregamos alguna columna agrupando las variables plantas en plantas bajas y plantas altas para un mejor análisis.

```{r}
mhp <- mhp %>%
  mutate(plantas = ifelse(grepl("planta [2-9]|[1-4]\\d|50", house_type), "plantas altas", "plantas bajas"))
```

También reamos una columna con el precio medio por distrito en la que salen las variables caro, medio o barato en función del precio del distrito.
...
percentiles <- quantile(mhp$precio_distrito, probs = c(0, 1/3, 2/3, 1))

# Asignar etiquetas
mhp$precio_distrito <- cut(mhp$precio_distrito, breaks = percentiles, labels = c("barato", "medio", "caro"), include.lowest = TRUE)
```

Volvemos a realizar la división del dataset.
...
dummy_plantas <- model.matrix(~0 + plantas, data = train)
dummy_distrito <- model.matrix(~0 + precio_distrito, data = train)
```

```{r}
dummy_data <- cbind(train[, c("rooms", "m2")],dummy_exterior,dummy_garaje,dummy_ascensor,dummy_distrito,dummy_plantas)
```
...
summary(pca_result)
```
Observamos que con PC5 ya llegamos al 80% de la proporción de varianza explicada pero vemos algunos saltos interesantes. Para ver cuantas componentes explican el modelo hacemos la regla de codo.
...
```
Al observar el gráfico vemos que la forma del codo sería en la 5,6 aunque no es un gráfico muy claro. 
...
```{r}
glm_mhp = glm(formula = cara ~ exterior + garage + elevator + rooms + precio_distrito + plantas,
                 data = train_glm, 
                 family = binomial)
summary(glm_mhp)
```
...
library(rpart.plot)
library(rattle)
arbol <- rpart(cara ~ garage + elevator + rooms + plantas + precio_distrito, data = mhp_train, control = rpart.control(minsplit = 1))
...
```{r}
mhp_train$cara <- ifelse(mhp_train$cara == "cara", 1, 0)
```
